...
def employer_chatbot(skill_query: str, top_k: int):
    ...
    try:
        ...
        return f"An error occurred while processing your request: {str(e)}", pd.DataFrame(), None, None
...
def admin_upload_resume(files):
    ...
    try:
        ...
        allowed_ext = {".pdf", ".docx", ".txt", ".doc", ".odt", ".csv"}
        uploaded = []
        rejected = []
        ...
        return f"An error occurred during upload: {str(e)}", list_uploaded_resumes()
...
def list_uploaded_resumes():
    try:
        ...
        return pd.DataFrame(columns=["File Name", "File Type", "Size KB"])
